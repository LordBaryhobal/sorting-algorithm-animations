...
class Sorter:
    ...
    def sort(self):
        for interval in range(len(self.original)-1, 0, -1):        
            for i in range(len(self.original)-interval):
                step = {
                    "ia": i,
                    "ib": i+interval,
                    "a": self.list[i],
                    "b": self.list[i+interval],
                    "swap": False,
                    "list_start": list(self.list)
                }
                if self.list[i] > self.list[i+interval]:
                    step["list_start"][i] = ""
                    step["list_start"][i+interval] = ""
                    step["swap"] = True
                    self.list[i], self.list[i+interval] = self.list[i+interval], self.list[i]
                    changed = True
    ...
    def render(self):
        ...
        while self.step_i < len(self.steps):
            ...
            if self.output_video:
                pygame.image.save(self.win, os.path.join(tempdir.name, f"{frame:06d}.jpg"))
                frame += 1
    ...
    def make_video(self, tempdir):
        ...
        os.system(f"ffmpeg -framerate {self.FPS} -pattern_type glob -i '{tempdir.name}/*.jpg' -c:v libx264 -pix_fmt yuv420p {path}.mp4")
